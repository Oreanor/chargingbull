...
- **Карта на телефоне рисует не настоящие дома, а коробки.** Каждый футпринт
  Mapbox заранее сведён к минимальному повёрнутому прямоугольнику — 4 точки
  вместо 8–20, — и там же, в бейке, посчитаны подсветка NYSE и x-ray, поэтому
  телефонный путь не дёргает `queryRenderedFeatures` и не держит feature-state.
  Пекётся офлайн: `npm run bake:boxes` → `public/chapters/bull/data/building-boxes.json`
  (коммитится; пересобирать только если поменялись близкие стопы камеры или
  полигоны NYSE). Посмотреть любой из двух городов на любом экране —
  `?city=boxes` / `?city=tiles`. Детали и границы покрытия — в шапке
  `scripts/bake-building-boxes.mjs` и `src/engine/mapBuildings.ts`
